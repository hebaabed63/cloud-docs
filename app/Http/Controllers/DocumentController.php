...
class DocumentController extends Controller
{
    ...
    if (!in_array($extension, ['pdf', 'doc', 'docx'])) {
        return back()->withErrors(['نوع الملف غير مدعوم.']);
    }

    try {
        // تحميل الملف مؤقتًا
        $tempPath = storage_path('app/' . uniqid() . '.' . $extension);
        ...
        $document = new Document();
        $document->filename = basename($url);
        $document->title = $title ?? 'No Title';
        $document->file_path = $path;
        $document->size = $size;
        $document->public_id = $publicId;
        ...
        return redirect()->back()->with('success', 'تم رفع المستند من الرابط بنجاح!');
    } catch (\Exception $e) {
        return back()->withErrors(['فشل رفع الملف: ' . $e->getMessage()]);
    }
    }
